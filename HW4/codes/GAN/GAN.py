...
class Generator(nn.Module):
    ...
    def forward(self, z):
        ...
        *   Arguments:
        ...
        if os.getenv('MLP') == '1':
            z = z.squeeze()
            return self.decoder(z).view(-1, self.num_channels, 32, 32)
...
class Discriminator(nn.Module):
    def __init__(self, num_channels, hidden_dim):
        ...
        if os.getenv('MLP') == '1':
            ...
            self.clf = nn.Sequential(
                nn.Linear(self.num_channels * 32 * 32, self.hidden_dim),
        ...
        elif os.getenv('replace') == '1': # replace LeakyReLU with ReLU
            print('replace LeakyReLU with ReLU')
            self.clf = nn.Sequential(
                # input is (num_channels) x 32 x 32
                nn.Conv2d(num_channels, hidden_dim, 4, 2, 1, bias=False),
    ...
    def forward(self, x):
        if os.getenv('MLP') == '1':
            x = x.view(-1, self.num_channels * 32 * 32).contiguous()
