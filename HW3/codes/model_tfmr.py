...
class TransposeLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(*size_out)
        return x # output_size[-1] = self.nf
...
class TfmrAttention(nn.Module):
    def __init__(self, config):
        ...
        self.scale_attn_weights = config.scale_attn_weights # bool
    ...
    def forward(
        ...
        query = self._split_heads(query, self.num_heads, self.head_dim) # size = (batch_size, num_heads, sequence_length, head_dim)
...
class TfmrModel(nn.Module):
    ...
    def forward(
        ...
        inputs_embeds = self.wte(input_ids) # (batch_size, sequence_length, hidden_size)
        ...
        # TODO END
        hidden_states = inputs_embeds + position_embeds # (batch_size, sequence_length, hidden_size)
...
class TfmrLMHeadModel(nn.Module):
    ...
    def forward(
        ...
        hidden_states = transformer_outputs["last_hidden_state"] # (batch_size, sequence_length, hidden_size)
        lm_logits = self.lm_head(hidden_states) # (batch_size, sequence_length, vocab_size)
