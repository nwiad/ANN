...
def load_model(pretrained_dir, model_name="pretrained_ckpt.bin"):
    ...
    if os.path.exists(model_path):
        ...
        config_path = os.path.join(pretrained_dir, f"config_{args.test}.json") if args.test is not None else os.path.join(pretrained_dir, "config.json")
...
def main():
    ...
    if args.test is None:
        if args.pretrain_dir is None:
            ...
            if args.name == "Tfmr-finetune-1-6-12":
                new_h = nn.ModuleList(nn.Sequential() for _ in range(3))
                new_h[0] = model.transformer.h[0]
                new_h[1] = model.transformer.h[5]
                new_h[2] = model.transformer.h[11]
                model.transformer.h = new_h
        model.to(device)
        print(model)
        model.train()
        ...
        if args.name == "Tfmr-finetune-1-6-12":
            ...
            with open(f"output_Tfmr-finetune-1-6-12_{args.decode_strategy}_{args.temperature}_{args.top_p}.txt", "a") as fout:
            ...
            with open(f"output_Tfmr-finetune-1-6-12_{args.decode_strategy}_{args.temperature}_{args.top_p}.txt", "a") as fout:
                fout.write("test_set, perplexity {:.2f}\n".format(test_ppl))
                fout.write("test_set, forward BLEU-4 {:.3f}, backward BLEU-4 {:.3f}, harmonic BLEU-4 {:.3f}\n".format(eval_result["fw-bleu-4"], eval_result["bw-bleu-4"], eval_result["fw-bw-bleu-4"]))
        ...
        result = model.inference(device=device, PAD_ID=PAD_ID, 
            batch_size=args.batch_size, maxlen=args.maxlen, decode_strategy=args.decode_strategy, temperature=args.temperature, top_p=args.top_p)
        with open(f"output_{args.test}_{args.decode_strategy}_{args.temperature}_{args.top_p}.txt", "a") as fout:
            for k, output in enumerate(result):
                out = tokenizer.decode(output)
                print(k, out)
                fout.write(out + "\n")
